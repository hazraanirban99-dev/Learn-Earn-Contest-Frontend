...
const PortalInput = React.memo(({ label, id, value, onChange, placeholder, icon: Icon, subNote, type = 'text', rightElement, borderColor }) => (
  <div className="flex flex-col gap-1.5">
    ...
    <div className="relative group">
      <div className={`flex items-center bg-[#f1f8e8] border-2 ${borderColor || 'border-transparent focus-within:border-[#8cc63f]/40'} focus-within:bg-white dark:bg-gray-800 rounded-2xl transition-all shadow-sm overflow-hidden`}>
        {Icon && (
        ...
        <input
          ...
          className={`flex-1 py-4 outline-none bg-transparent text-slate-800 dark:text-gray-100 font-bold text-sm placeholder-gray-300 ${!Icon ? 'px-5' : 'pr-4'}`}
        />
