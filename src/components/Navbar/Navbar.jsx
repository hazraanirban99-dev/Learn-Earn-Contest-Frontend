...
import { FiMenu, FiX, FiBell, FiSettings, FiRefreshCw, FiCheck, FiXCircle, FiUser, FiLogOut, FiTrash2, FiEdit, FiHome, FiAward, FiFolder, FiMoreVertical, FiSun, FiMoon } from 'react-icons/fi';
...
const Navbar = ({ showAuth = true }) => {
  ...
  const navLinks = [
    ...
    { 
      label: 'Participants', 
      path: user?.role === 'student' ? '/student/dashboard' : '/participants', 
      id: 'participants' 
    },
    ...(user && user.role === 'admin' ? [{ label: 'Admin', path: '/admin/dashboard', id: 'admin' }] : []),
  ];
  ...
  return (
    <>
    <nav className="fixed w-full top-0 z-[100] bg-white/ dark:bg-gray-800/0 dark:bg-gray-900/90 backdrop-blur-xl border-b border-gray-100/50 dark:border-gray-700/50 shadow-sm">
        ...
        <div className="hidden lg:flex items-center gap-10">
          {navLinks.map((link) => (
            <Link
              ...
              className={`relative py-1 text-[13px] font-black uppercase tracking-[0.1em] transition-all duration-300 hover:text-slate-900 dark:text-gray-100 dark:hover:text-white ${
                (location.pathname === link.path || (link.id === 'participants' && user?.role === 'student' && location.pathname === '/student/dashboard')) 
              ...
              <div className="flex items-center gap-3 sm:gap-6">
                ...
                {user?.role === 'admin' && (
                  <div className="hidden lg:flex items-center gap-4 border-r border-gray-100 dark:border-gray-700 pr-6 mr-2">
                                 ...
                                 <div key={idx} className="p-4 border-b border-gray-50 dark:border-gray-700 hover:bg-gray-50/ dark:bg-gray-800/ dark:hover:bg-gray-700/50 transition-colors">
                                   <div className="flex gap-3">
                                      <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${
                                          (n.type === 'TEAM_CREATION_REQUEST' || n.type === 'TEAM_INVITE') ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-500' : 'bg-purple-50 dark:bg-purple-900/30 text-purple-500'
                                      }`}>
                                        <FiUser size={16} />
                                      </div>
                    ...
                    {/* Settings */}
                    <button 
                      onClick={() => toast.info("Scholastic tools are ready.")}
                      className="p-2 text-gray-400 hover:text-[#8cc63f] hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 rounded-xl transition-all"
                    >
                      <FiSettings size={18} />
                    </button>
                  </div>
                ...
                <div className="relative hidden lg:block" ref={profileDropdownRef}>
                  ...
                  {isProfileOpen && (
                    <div className="absolute top-full right-0 mt-3 w-60 sm:w-64 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden z-[110] animate-in fade-in slide-in-from-top-2 duration-300">
                      ...
                      <div className="p-2">
                        ...
                        {user.role === 'student' && (
                           <button onClick={() => setIsProfileModalOpen(true)} className="w-full flex items-center gap-3 px-4 py-3 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 rounded-xl text-xs font-bold transition-colors">
                              <FiUser size={16} /> View Profile
                           </button>
                        )}
                        {user.role === 'student' && (
                           <button onClick={handleDeleteAccount} className="w-full flex items-center gap-3 px-4 py-3 text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl text-xs font-bold transition-colors">
                              <FiTrash2 size={16} /> Delete Account
                           </button>
                        )}
                        <button onClick={logout} className="w-full flex items-center gap-3 px-4 py-3 text-[#fbc111] hover:text-[#e0ad0c] hover:bg-yellow-50/50 dark:hover:bg-yellow-900/20 rounded-xl text-xs font-bold transition-colors border-t border-gray-50 dark:border-gray-700 mt-1 pt-3">
    ...
    <AnimatePresence>
      {isSidebarOpen && user?.role === 'student' && (
        <>
           <motion.div 
             initial={{ opacity: 0 }}
             animate={{ opacity: 1 }}
           ...
           <motion.aside 
             ...
             className="fixed top-0 left-0 h-full w-[280px] bg-white dark:bg-gray-900 z-[1001] shadow-2xl border-r border-gray-100 dark:border-gray-700 flex flex-col p-8"
           >
              <div className="flex items-center justify-between mb-12">
                  <Logo size="md" />
                  <button onClick={() => setIsSidebarOpen(false)} className="p-2 text-gray-400 hover:text-slate-900 dark:text-gray-100 dark:hover:text-gray-100 transition-colors">
                      <FiX size={24} />
                  </button>
              </div>

              <div className="flex flex-col gap-6">
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 opacity-60">Scholastic Navigator</p>
                  <div className="flex flex-col gap-2">
