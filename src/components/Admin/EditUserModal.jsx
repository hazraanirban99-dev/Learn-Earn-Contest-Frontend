...
const EditUserModal = ({ user, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-[700px] rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row relative animate-in zoom-in-95 duration-300 max-h-[95vh] md:max-h-[85vh] overflow-y-auto">
        
        {/* Left Sidebar Info */}
        <div className="bg-[#f2f7e7] w-full md:w-[260px] p-8 flex flex-col justify-between hidden md:flex shrink-0">
          <div>
            <div className="bg-white w-10 h-10 rounded-xl flex items-center justify-center text-[#8cc63f] mb-6 shadow-sm border border-gray-100/50">
              <FiEdit3 size={20} />
            </div>
            <h3 className="text-2xl font-black text-slate-800 tracking-tight leading-none mb-4">Edit Profile</h3>
        ...
        <div className="flex-1 p-5 sm:p-8 md:p-10 relative">
          <button 
            onClick={onClose}
            className="absolute top-6 right-6 p-2 text-gray-400 hover:text-slate-900 bg-gray-50 hover:bg-gray-100 rounded-full transition-all"
          >
            <FiX size={18} />
          </button>
          ...
          <div className="flex items-center gap-4 mb-8 pb-8 border-b border-gray-100">
            ...
            <div>
              <h4 className="text-lg font-black text-slate-900 tracking-tight">{formData.name}</h4>
          ...
          <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-3 mt-8">
            ...
            <button 
              onClick={onClose}
              className="px-6 py-3 sm:py-3.5 text-sm font-bold text-gray-600 hover:text-slate-900 transition-colors w-full sm:w-auto text-center"
            >
              Cancel
            </button>
            <button 
