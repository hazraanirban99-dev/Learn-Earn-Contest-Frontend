         ...
         <PageTransition>
            ...
            <section className="py-12 bg-white/ dark:bg-gray-800/ overflow-hidden">
               ...
               {loading ? (
                  ...
                  <div className="relative h-[500px] md:h-[480px] max-w-[1440px] mx-auto flex items-center justify-center pt-8 overflow-hidden lg:overflow-visible">
                     <div
                        className="absolute left-[5%] xl:left-[15%] w-[250px] md:w-[320px] transform scale-[0.7] opacity-10 md:opacity-20 blur-[4px] transition-all duration-1000 select-none z-10 pointer-events-none hidden lg:block transform-gpu"
                        style={{ transform: 'translateX(-50%)' }}
                     >
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-[48px] shadow-sm flex flex-col gap-4">
                           <img loading="lazy" src={contests[getCardIndex(-1)]?.image} className="w-full h-[180px] object-cover rounded-[36px] grayscale" alt="Contest Preview" />
                     ...
                     <AnimatePresence mode={isMobile ? "sync" : "popLayout"} initial={false}>
                        <motion.div
                           key={currentIndex}
                           initial={{ opacity: 0, x: '100%', scale: isMobile ? 1 : 0.95 }}
                           animate={{ opacity: 1, x: 0, scale: 1 }}
                           exit={{ opacity: 0, x: '-100%', scale: isMobile ? 1 : 0.95 }}
                           transition={isMobile ? mobileTransition : desktopTransition}
                           className="relative w-full max-w-[95%] md:max-w-[300px] lg:max-w-[330px] z-30 px-2 sm:px-4 transform-gpu will-change-transform"
                           onClick={() => handleContestClick(contests[currentIndex]?.id)}
                        >
                           <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-[40px] md:rounded-[48px] shadow-2xl border-[6px] md:border-[8px] border-transparent bg-clip-padding relative h-[420px] md:h-[420px] flex flex-col items-center justify-between cursor-pointer group">
                     ...
                     {/* RIGHT CARD (PREVIEW) */}
                     <div
                        className="absolute right-[5%] xl:right-[15%] w-[250px] md:w-[320px] transform scale-[0.7] opacity-10 md:opacity-20 blur-[4px] transition-all duration-1000 select-none z-10 pointer-events-none hidden lg:block transform-gpu"
                        style={{ transform: 'translateX(50%)' }}
                     >
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-[48px] shadow-sm flex flex-col gap-4">
                           <img loading="lazy" src={contests[getCardIndex(1)]?.image} className="w-full h-[180px] object-cover rounded-[36px] grayscale" alt="Contest Preview" />
